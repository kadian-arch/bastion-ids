...
CICIDS_MAP = {
    ...
    # response_body_len has no CICIDS/CICFlowMeter equivalent — left to the
    # "fill missing UNSW features with defaults" branch in convert_cicids().
    #
    # sload/dload: UNSW defines these as bits/sec throughput, and CICIDS's
    # Fwd/Bwd Pkts/s are a packet rate — a real unit mismatch, kept anyway.
    # Swapping in a true bits/sec computation (bytes*8/dur) was tried and
    # measured on a labeled CICIDS test set (uploaded_datasets/
    # test_cicids_bruteforce_heavy.csv, 3000 flows): it collapsed ML-ensemble
    # attack recall from 89% to 0%. Brute-force flows here are rapid small
    # packets — high rate, low total bytes — so true throughput reads near-
    # zero and looks benign, while packet rate is exactly the signal the
    # ensemble keys off. Do not "fix" this without retraining the models on
    # CICIDS-native feature distributions first.
