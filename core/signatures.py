...
class BastionSignatureEngine:
    ...
    def __init__(self):
        ...
        self.disabled_sids = {
            ...
            # ── PCRE-critical rule that slips past the generic PCRE guard ────────
            # Confirmed false positive 2026-07-25: fired repeatedly on a plain
            # curl/nikto request to an ordinary basic-HTML test page, direction
            # server-to-client, nothing exploit-related in flight. The content
            # keywords (<script, document.getelementbyid(, .scroll, Set) are each
            # individually common in ordinary JavaScript and pass both PCRE-guard
            # checks (combined length well over 8 bytes, first token isn't a
            # generic protocol opener), so the guard never engages. The real
            # discriminator was always the PCRE: a specific VBScript
            # "Class ... End Class ... scroll = New Class" structure, which
            # Bastion's engine cannot evaluate.
            2034578,   # ET EXPLOIT IE Scripting Engine Memory Corruption Vulnerability M2 (CVE-2019-0752): content keywords alone match ordinary scroll-handling JavaScript; PCRE (the real discriminator) unevaluated
        }
