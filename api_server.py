...
def _save_alert(alert: dict):
    ...
    with _alerts_lock:
        try:
            ...
            print(f"[_save_alert DIAG] buffered verdict={alert.get('verdict')!r} "
                  f"session={alert.get('session')!r} buffer_len={len(_alert_buffer)}", flush=True)
            now = time.time()
            if (len(_alert_buffer) >= _alert_flush_size or
                ...
                if os.path.exists(CONFIG["ALERTS_FILE"]):
                    try:
                        ...
                        print(f"[_save_alert DIAG] existing-file read failed, starting fresh: "
                              f"{type(_re).__name__}: {_re}", flush=True)
                        existing = []
                existing.extend(_alert_buffer)
                _flushed_count = len(_alert_buffer)
                _alert_buffer = []
                ...
                print(f"[_save_alert DIAG] FLUSHED {_flushed_count} alerts to disk, "
                      f"file now has {_alerts_disk_count} total", flush=True)
        except Exception as _e:
            import traceback as _tb
            print(f"[_save_alert DIAG] EXCEPTION, alert lost: {type(_e).__name__}: {_e}\n"
...
async def get_interfaces():
    ...
    # No interface is flagged as "recommended". This app has no way to know
    # what network topology or capture scenario the user actually has (Wi-Fi,
    # wired LAN, a VM-only lab network, etc.), so it does not guess. Every
    # interface keeps "recommended": False; the user picks.
    result.sort(key=lambda i: (
        i["virtual"], not i["is_up"], not i["ip"],
    ))
    return result
...
async def capture_stats():
    ...
    if is_sniffing and arp_only:
        # Flag that the current interface looks wrong (ARP-only traffic is a
        # strong signal of a virtual/NAT adapter seeing only broadcasts), but
        # do not guess which other interface is "better". This app has no
        # way to know the user's actual network topology, so it does not
        # name or rank an alternative. The user decides.
        iface_warning = (
            f"Only ARP packets captured on '{current_interface}'. This is likely a "
            f"virtual/NAT adapter that sees only broadcast traffic. Check the interface "
            f"list and select the one that matches the network you actually want to monitor."
        )
...
async def analyze_batch_flows(request: Request,
                               x_authority: str = Header(None)):
    """
    Analyze multiple flows using vectorized ML inference (offline batch-analysis path).
    Body: { "flows": [{feature_dict}, ...], "force_ml": true }
    Returns: { "n_flows": N, "results": [{flow_index, verdict, confidence, engine}, ...] }
    """
    if _lockdown_active:
        raise HTTPException(status_code=423, detail="SYSTEM LOCKDOWN ACTIVE")
    ...
    # Default force_ml=True for batch path: offline classification, not real-time blocking
    force_ml = bool(body.get("force_ml", True))

    def _batch_analyze(flow_list, use_force_ml):
        import functools as _ft
        import numpy as _np
        from utils.feature_bridge import bridge
        ...
        n = len(flow_list)
        results = [None] * n
        ml_needed = list(range(n))

        # Layer 1: optional per-flow signature scan
        if not use_force_ml:
            ml_needed = []
            for i, fd in enumerate(flow_list):
                df_s = pd.DataFrame([{k: (v if v is not None else 0) for k, v in fd.items()}])
                try:
                    v, c, src = engine.analyze_flow(df_s, None)
                    if src not in ("BASTION_CLEAN", None) and str(v).upper() == "NORMAL":
                        results[i] = ("NORMAL", 0.0, "BASTION_CLEAN")
                    elif src == "SIGNATURE_DB":
                        results[i] = (v, c, src)
                    else:
                        ml_needed.append(i)
                except Exception:
        ...
        # Build batch DataFrame for all ML-needed flows
        rows = [{k: (v if v is not None else 0) for k, v in flow_list[i].items()}
                for i in ml_needed]
        df_batch = pd.DataFrame(rows)

        try:
            df_clean, _ = bridge(df_batch)
            X_ml, X_dl = engine._transform(df_clean)
        except Exception:
            for i in ml_needed:
                results[i] = ("NORMAL", 0.0, "BASTION_CLEAN")
            return results

        # Layer 2: vectorized predict_proba across all N flows in one call per model
        proba_rf = proba_xgb = proba_cat = None
        if engine.rf  is not None:
            try: proba_rf  = engine.rf.predict_proba(X_ml)
            except Exception: pass
        if engine.xgb is not None:
            try: proba_xgb = engine.xgb.predict_proba(X_ml)
            except Exception: pass
        if engine.cat is not None:
            try: proba_cat = engine.cat.predict_proba(X_ml)
            except Exception: pass

        # Layer 3: vectorized DNN inference in one batch call
        proba_dl = None
        if engine.dl is not None:
            try:
                X_in = X_ml
                try:
                    X_in = X_dl if X_dl is not None else X_ml
                    proba_dl = engine.dl.predict(X_in, verbose=0, batch_size=len(ml_needed))
                except Exception:
                    proba_dl = engine.dl.predict(X_ml, verbose=0, batch_size=len(ml_needed))
            except Exception:
                pass

        # Apply fusion per row (just numpy indexing, no model calls)
        for j, orig_i in enumerate(ml_needed):
            experts = []
            for name, proba in [("RF", proba_rf), ("XGB", proba_xgb), ("CAT", proba_cat)]:
                if proba is None:
                    continue
                try:
                    idx  = int(_np.argmax(proba[j]))
                    conf = float(_np.max(proba[j]))
                    lbl  = engine._decode(idx)
                    experts.append({"name": name, "label": lbl, "conf": conf})
                except Exception:
                    continue

            malicious = [e for e in experts
                         if e["label"].upper() != "NORMAL" and e["conf"] >= _ml_threshold]
            if len(malicious) < 2:
                soft = [e for e in experts
                        if e["label"].upper() != "NORMAL"
                        and e["conf"] >= max(0.50, _ml_threshold - 0.10)]
                if len(soft) >= 2:
                    lbls = [e["label"].upper() for e in soft]
                    if lbls.count(lbls[0]) >= 2:
                        malicious = [e for e in soft if e["label"].upper() == lbls[0]]
            ...
            if len(malicious) >= 2 or any(e["conf"] >= 0.92 for e in malicious):
                top = max(malicious, key=lambda x: x["conf"])
                results[orig_i] = (top["label"].upper(), top["conf"], "ML_ENSEMBLE")
                continue

            # Layer 3: DNN row verdict
            if proba_dl is not None:
                try:
                    dl_idx  = int(_np.argmax(proba_dl[j]))
                    dl_conf = float(_np.max(proba_dl[j]))
                    dl_lbl  = engine._decode(dl_idx)
                    if dl_lbl.upper() != "NORMAL" and dl_conf >= 0.50:
                        results[orig_i] = (dl_lbl.upper(), dl_conf, "DL-SENSEI")
                        continue
                except Exception:
                    pass

            # Layer 4: Anomaly (slice keeps 2D shape for the scorer)
            try:
                ar = engine._anomaly_check(X_ml[j : j + 1])
                if ar is not None:
                    score, av = ar
                    results[orig_i] = (av, score, "ANOMALY")
                    continue
            except Exception:
                pass

            results[orig_i] = ("NORMAL", 0.0, "BASTION_CLEAN")
    ...
    try:
        ...
        raw_results = await loop.run_in_executor(
            None, _batch_analyze, flows, force_ml
        )
    except Exception as ex:
        raise HTTPException(status_code=500, detail=f"Batch analysis failed: {ex}")

    out = []
    for i, r in enumerate(raw_results):
        if r is None:
            r = ("NORMAL", 0.0, "BASTION_CLEAN")
        verdict, conf, eng = r
        conf_norm = float(conf) if float(conf) <= 1.0 else float(conf) / 100.0
        out.append({
            "flow_index": i,
    ...
    return {"n_flows": len(flows), "results": out}
